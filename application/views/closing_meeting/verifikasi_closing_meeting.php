...
<body <?php echo $body_parameter; ?>>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-footer" id="folder_dokumen-footer">
                    ...
                    <button type="button" class="btn btn-primary btn-sm" id="folder_dokumen-kirim"><i class="fa fa-paper-plane me-2"></i> <span id="folder_dokumen-label_btn"></span></button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function load_data() {
            ...
            data['filter'] = filter;
            data['for'] = 'verifikasi_closing_meeting';
                    ...
                    if (list_data.length > 0) {
                        for (var i = 0; i < list_data.length; i++) {
                            ...
                            var list_folder =
                                ...
                                '<a href="javascript:;" onclick="buka_folder(' + i + ')" class="menu-link px-3"><i class="fa fa-check-circle text-primary pe-3"></i> Verifikasi Closing Meeting</a>' +
                                '</div>';
                    ...
                    success: function(list_dokumen) {
                        ...
                        if (list_dokumen.length > 0) {
                            ...
                            list_dokumen.map((item, i) => {
                                ...
                                if (item.dokumen[0].status == 3) {
                                    menunggu++;
                                    btn_action = '<a href="' + base_url + 'page/approval_dokumen/?id_dokumen=' + item.dokumen[0].id_closing_meeting_dokumen + '&file=' + base_url + item.dokumen[0].path_file + '&url_verifikasi=closing_meeting_dokumen/verifikasi_dokumen" onclick="$(\'#folder_modal\').modal(\'hide\')" target="_blank" class="btn btn-sm btn-primary me-2"><i class="fa fa-check me-2"></i> Buka & Verifikasi File</a>';
                                } else if (item.dokumen[0].status == 1) {
                                    setuju++;
                                    alasan_verifikasi = render_badge('light-success', 'Dokumen Disetujui');
                                } else {
                                    ditolak++;
                            ...
                            if (menunggu > 0) {
                                $("#folder_dokumen-footer").hide();
                            } else {
                                $("#folder_dokumen-footer").show();
                                if (ditolak > 0) {
                                    $("#folder_dokumen-action").val('verifikator');
                                    $("#folder_dokumen-label_btn").html('Kirim Ke Verifikator');
                                } else {
                                    //cek apakah saat ini adalah dokumen kontrol atau bukan...
                                    var as = '<?php echo $konten['as']; ?>';
                                    if (as == 'dokumen kontrol') {
                                        //jika sebagai dokumen kontrol, kirim ke koordinator...
                                        $("#folder_dokumen-action").val('koordinator');
                                        $("#folder_dokumen-label_btn").html('Kirim Ke Koordinator');
                                    } else {
                                        //jika sebagai koordinator, kirim ke pelanggan...
                                        $("#folder_dokumen-action").val('pelanggan');
                                        $("#folder_dokumen-label_btn").html('Kirim Ke Pelanggan');
                                    }
                                }
                            }
        ...
        $("#folder_dokumen-kirim").click(function() {
            var action = $("#folder_dokumen-action").val();

            var pertanyaan = "Apakah Anda yakin akan mengirimkan dokumen ke " + action + "?";
        ...
        function goto_kirim(action) {
            ...
            $.ajax({
                type: "POST",
                url: '<?php echo base_url(); ?>closing_meeting_dokumen/goto_kirim_' + action,
                data: 'data_send=' + encodeURIComponent(JSON.stringify(data)),
                cache: false,
                dataType: "json",
                success: function(data) {
                    ...
                    } else if (data.sts == 'dokumen_belum_disetujui') {
