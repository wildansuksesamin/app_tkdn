...
<body <?php echo $body_parameter; ?>>
                            ...
                            <div id="kt_app_content_container" class="app-container container-fluid">
                                ...
                                <div class="card card-flush h-lg-100 tabel_zone" id="tabel_dokumen_penawaran">
                                    ...
                                    <div class="card-body p-lg-17" style="padding-top: unset!important;">
                                        <!--begin::Layout-->
                                        <div id="data_zona_dokumen_penawaran">
                                            <div class="d-flex flex-column flex-lg-row">
                                                <!--begin::Content-->
                                                <div class="flex-lg-row-fluid me-0">
                                                    <div class="table-responsive">
                                                        <table class="table table-sm table-striped gy-5" id="data_dokumen_penawaran">
                                                            <thead>
    ...
    <script>
        ...
        function load_data() {
            ...
            data['filter'] = filter;
            data['from'] = 'buat_penawaran';

            ajax_request = $.ajax({
                type: "POST",
                url: '<?php echo base_url(); ?>rab/load_data',
                ...
                success: function(result) {
                    ...
                    if (list_data.length > 0) {
                        for (var i = 0; i < list_data.length; i++) {
                            var label_action = 'Buat Penawaran';
                            if (list_data[i].status_pengajuan == 9)
                                label_action = 'Revisi Penawaran';
                            var action = '<a href="' + base_url + 'page/proses_buat_penawaran/' + list_data[i].id_rab + '" class="btn btn-icon-primary btn-text-primary" style="padding: unset; background: unset"><span class="svg-icon svg-icon-3"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path opacity="0.3" d="M19 22H5C4.4 22 4 21.6 4 21V3C4 2.4 4.4 2 5 2H14V4H6V20H18V8H20V21C20 21.6 19.6 22 19 22Z" fill="currentColor"/><path d="M15 8H20L14 2V7C14 7.6 14.4 8 15 8Z" fill="currentColor"/></svg></span> ' + label_action + '</a>';

                            rangkai += '<tr>' +
                                '<td>' + (((page - 1) * jml_data) + i + 1) + '.</td>' +
                                '<td>' +
                                '<div class="text-gray-400 fw-semibold d-block fs-7">' + reformatDate(list_data[i].tgl_penyusunan_rab, 'DD MMMM YYYY') + '</div>' +
                                ...
                                '</td>' +
                                '<td>' +
                                '<div class="text-gray-800 fw-bold mb-1 fs-6">' + coverMe(list_data[i].nama_tipe_permohonan) + '</div>' +
                                '<div class="text-gray-400 fw-semibold d-block fs-7">' + coverMe(list_data[i].nama_produk_jasa) + '</div>' +
                                render_badge_tipe_pengajuan(list_data[i].tipe_pengajuan) +
                                '</td>' +
                                '<td class="text-gray-600 fw-bold fs-6">Rp ' + rupiah(list_data[i].nilai_kontrak) + '</td>' +
                    ...
                    if (rangkai) {
                        $("#empty_state").remove();
                        $("#data_zona_dokumen_penawaran").show();

                        $("#data_dokumen_penawaran tbody").html(rangkai);
                    } else {
                        if (page == 1 && filter == '') {
                            create_empty_state("#data_zona_dokumen_penawaran");
                        } else
                            $("#data_dokumen_penawaran tbody").html('');
                    }
