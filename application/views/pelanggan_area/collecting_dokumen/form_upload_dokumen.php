...
<body <?php echo $body_parameter; ?>>
    ...
    <div class="modal fade" id="folder_dokumen_modal" tabindex="-1" role="dialog" aria-hidden="true">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <div class="modal-title">
                        ...
                        <a href="" id="folder_dokumen-download_template" class="btn btn-sm btn-light-primary mt-3" download>Download Template</a>
                ...
                <div class="modal-body">
                    ...
                    <div id="textarea_zone" class="input_zone">
                        <textarea name="file_textarea" id="file_textarea" class="form-control form-control-solid"></textarea>

                        <button type="button" id="simpan_file_textarea" class="btn btn-primary mt-4">
                            <span class="indicator-label"><i class="fa-solid fa-floppy-disk me-2 fs-3"></i> Simpan</span>
                            <span class="indicator-progress">Loading...
                                <span class="spinner-border spinner-border-sm align-middle ms-2"></span></span>
                        </button>
                    </div>
                    <form id="dropzone" class="form input_zone" action="#" method="post">
    ...
    <?php $this->view('pelanggan_area/include/bom'); ?>
    <script>
        var id_status = '<?php echo $konten['opening_meeting']->id_status; ?>';
        ...
        myDropzone.on("success", function(file, result, xhr) {
            ...
            } else if (result.sts == 'tidak_sesuai_format') {
                errorNode.className = "dz-error-message";
                var response = JSON.parse('<?php echo alert('tidak_sesuai_format'); ?>');
                message = response.message;
        ...
        $("#simpan_file_textarea").click(function() {
            var file_textarea = $("#file_textarea").val();
            var id_opening_meeting = $("#folder_dokumen-id_opening_meeting").val();
            var id_nama_file = $("#folder_dokumen-id_nama_file").val();
            var token = $("#folder_dokumen-token").val();

            if (!file_textarea || !id_opening_meeting || !id_nama_file || !token) {
                ...
                var data = new Object;
                data['token'] = token;
                data['file_textarea'] = file_textarea;
                data['id_opening_meeting'] = id_opening_meeting;
                data['id_nama_file'] = id_nama_file;

                $.ajax({
                    type: "POST",
                    url: base_url + "pelanggan_area/collecting_dokumen/simpan_file_textarea",
                    ...
                    success: function(data) {
                        ...
                        if (data.sts == 1) {
                            ...
                            var response = JSON.parse('<?php echo alert('tidak_berhak_akses_data'); ?>');
                            swalAlert(response);
                        } else if (data.sts == 'mencapai_batas_simpan') {
                            var response = JSON.parse('<?php echo alert('mencapai_batas_simpan'); ?>');
                            response.message = response.message.replaceAll('{{nama_textarea}}', $("#folder_dokumen-nama_file").text());
                            swalAlert(response);
                        } else {
                            var response = JSON.parse('<?php echo alert('proses_gagal'); ?>');
                            swalAlert(response);
        ...
        function buka_folder(index) {
            ...
            $("#file_textarea").attr('placeholder', 'Tuliskan ' + data.nama_file.toLowerCase() + ' disini...')
            $("#folder_dokumen-keterangan").html(data.keterangan);
            $("#folder_dokumen-kolom_tambahan").val(data.kolom_tambahan);

            $("#folder_dokumen-download_template").hide();
            if (data.template) {
                $("#folder_dokumen-download_template").show();
                $("#folder_dokumen-download_template").attr('href', base_url + data.template);
            }
            ...
            // cek apakah yang dibuka adalah folder milik pelanggan atau pelanggan...
            if (id_status >= 7 && id_status < 15) {
                if (data.aktor == 'pelanggan') {
                    //jika milik pelanggan, maka tampilkan dropzone...
                    $('.input_zone').hide();
                    if (data.jns_file == 'textarea') {
                        $("#textarea_zone, .separator_dropzone").show();
                    } else {
                ...
                $("#dropzone, .separator_dropzone, #textarea_zone").hide();
        ...
        function rangkai_file() {
            ...
            if (files.length > 0) {
                for (var i = 0; i < files.length; i++) {
                    var verifikasi = '';
                    var btn_hapus = '<button type="button" class="btn btn-icon btn-danger btn-sm me-2" onClick="hapus_file(' + files[i].id_collecting_dokumen + ', \'' + files[i].value + '\')" title="Hapus file"><?php echo getSvgIcon('general/gen027', 'svg-icon svg-icon-muted svg-icon-3'); ?></button>';
                    if (files[i].status_verifikasi == 1) {
                        ...
                        //jika referensi adalah dari BOM, tidak perlu ditampilkan alasan penolakannya
                        //karena nanti didalam folder BOM akan ditampilkan masing-masing alasannya.
                        if (data.referensi != 'BOM') {
                    ...
                    var btn_upload_file = '';
                    if (data.referensi == 'BOM') {
                        btn_upload_file = '<div class="mt-2"><button class="btn btn-sm btn-primary" onclick="buka_folder_bom(' + files[i].id_collecting_dokumen + ')"><i class="fa fa-folder"></i> Folder BOM</button></div>';
                    }
                    ...
                    if (files[i].kolom_tambahan) {
                        ...
                        if (list_tambahan.length > 0) {
                            ...
                            for (var a = 0; a < list_tambahan.length; a++) {
                                kolom_tambahan += '<div class="col-sm-6"><div class="text-gray-600 fs-7">' + list_tambahan[a].label + '</div><div class="mb-3">' + list_tambahan[a].value + '</div></div>';
                            }
                            kolom_tambahan += '</div>';
                        }
                    }
                    ...
                    rangkai += '<tr>' +
                        ...
                        '</td>' +
                        '<td>' +
                        '<a href="' + base_url + files[i].resource + '" download class="btn btn-icon btn-primary btn-sm me-2" title="Download file"><i class="fa fa-download"></i></a>' +
                        (id_status >= 7 && id_status < 15 ? btn_hapus : '') +
                        '</td>' +
                        '</tr>';
                }
        ...
        function hapus_file(id_collecting_dokumen, nama_file) {
            var pertanyaan = "Apakah Anda yakin ingin mengapus file <strong>" + nama_file + "</strong>?";

            konfirmasi(pertanyaan, function() {
                proses_hapus_file(id_collecting_dokumen);
            });
        }

        function proses_hapus_file(id_collecting_dokumen) {
            ...
            $.ajax({
                type: "POST",
                url: '<?php echo base_url(); ?>pelanggan_area/collecting_dokumen/hapus_file',
                data: 'data_send=' + encodeURIComponent(JSON.stringify(data)),
                cache: false,
                dataType: "json",
                success: function(data) {
                    ...
                    if (list_data.length > 0) {
                        ...
                        for (var i = 0; i < list_data.length; i++) {
                            ...
                            rangkai += '<tr>' +
                                ...
                                '<div class=" mt-3"><span class="badge badge-' + (list_data[i].aktor == 'pelanggan' ? 'success' : 'secondary') + '"><i class="fa fa-user-circle text-gray-' + (list_data[i].aktor == 'pelanggan' ? '100' : '700') + ' me-3"></i> ' + list_data[i].aktor + '</span></div>' +
                                '</td>' +
                                '<td>' + status + '</td>' +
