...
<body <?php echo $body_parameter; ?>>
                    ...
                    <div class="d-flex flex-column flex-column-fluid">
                        ...
                        <div id="kt_app_content" class="app-content flex-column-fluid">
                            <div id="kt_app_content_container" class="app-container container-fluid">
                                ...
                                <div class="card card-flush h-lg-100 tabel_zone" id="tabel_pesan">
                                    ...
                                    <div class="card-body p-lg-17" style="padding-top: unset!important;">
                                        <!--begin::Layout-->
                                        <div id="data_zona_pesan">
                                            <div class="d-flex flex-column flex-lg-row">
                                                <!--begin::Content-->
                                                <div class="flex-lg-row-fluid me-0">
                                                    <div class="table-responsive">
                                                        <table class="table table-sm table-striped gy-5" id="data_pesan">
                                                            <thead>
                                                                <tr class="fw-semibold fs-6 text-gray-800 border-bottom-2 border-gray-200">
                                                                    <th>No.</th>
                                                                    <th>Tanggal</th>
                                                                    <th>Pelanggan</th>
                                                                    <th>Nomor Surat Reminder</th>
    ...
    <script>
        ...
        function load_data() {
            ...
            ajax_request = $.ajax({
                type: "POST",
                url: '<?php echo base_url(); ?>pelanggan_area/Pesan/load_data',
                ...
                success: function(result) {
                    ...
                    if (list_data.length > 0) {
                        for (var i = 0; i < list_data.length; i++) {
                            rangkai += '<tr>' +
                                ...
                                '<td>' + reformatDate(list_data[i].tgl_surat) + '</td>' +
                                '<td>' + coverMe(list_data[i].nama_badan_usaha + ' ' + list_data[i].nama_perusahaan) + '</td>' +
                                '<td>' + coverMe(list_data[i].nomor_surat) + '</td>' +
                                '<td>' + (list_data[i].read == 1 ? '' : render_badge('danger', 'Belum Dibaca')) + '</td>' +
                                '<td>' +
                                '<a href="' + base_url + 'pelanggan/lihat_pesan/' + list_data[i].id_pesan + '" class="btn btn-light-primary btn-icon-primary btn-sm">Lihat Dokumen <?php echo getSvgIcon('arrows/arr064', 'svg-icon svg-icon-1'); ?></a>' +
                                '</td>' +
                                '</tr>';
                        }
                    }
                    ...
                    if (rangkai) {
                        $("#empty_state").remove();
                        $("#data_zona_pesan").show();

                        $("#data_pesan tbody").html(rangkai);
                    } else {
                        if (page == 1 && filter == '') {
                            create_empty_state("#data_zona_pesan");
                        } else
                            $("#data_pesan tbody").html('');
                    }
